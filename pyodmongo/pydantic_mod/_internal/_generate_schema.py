...
from pydantic.config import ConfigDict, JsonEncoder
...
if TYPE_CHECKING:
    ...
    from pydantic.validators import FieldValidatorModes
...
class GenerateSchema:
    ...
    def _tuple_positional_schema(self, tp: Any, items_types: list[Any]) -> CoreSchema:
        items_schemas = [self.generate_schema(
            items_type) for items_type in items_types]
                ...
                for tp in (cls, *cls.__mro__):
                    ...
                    if extras_annotation is not None:
                        ...
                        if extra_items_type is not Any:
                            extra_validator = self.generate_schema(
                                extra_items_type)
                            break
    ...
    def _union_schema(self, union_type: Any) -> core_schema.CoreSchema:
        ...
        if len(choices) == 1:
            ...
            s = first_choice[0] if isinstance(
                first_choice, tuple) else first_choice
        else:
    ...
    def _type_alias_type_schema(
        ...
        with self.defs.get_schema_or_ref(origin) as (ref, maybe_schema):
            ...
            new_namespace = {
                **_typing_extra.get_cls_types_namespace(origin), **namespace}
    ...
    def _subclass_schema(self, type_: Any) -> core_schema.CoreSchema:
        ...
        if type_param == Any:
            ...
            args = self._get_args_resolving_forward_refs(
    ...
    def _prepare_annotations(self, source_type: Any, annotations: Iterable[Any]) -> tuple[Any, list[Any]]:
        ...
        """
        prepare = getattr(
            source_type, '__prepare_pydantic_annotations__', None)

        # make them immutable to avoid confusion over mutating them
        annotations = tuple(annotations)

        if prepare is not None:
            source_type, annotations = prepare(source_type, tuple(
                annotations), self._config_wrapper.config_dict)
    ...
    def _apply_annotations(
        ...
        while True:
            ...
            if annotation is None:
                continue
            prepare = getattr(
                annotation, '__prepare_pydantic_annotations__', None)
            if prepare is not None:
                previous = annotations[:idx]
                remaining = annotations[idx + 1:]
                new_source_type, remaining = prepare(source_type, tuple(
                    remaining), self._config_wrapper.config_dict)
...
_VALIDATOR_F_MATCH: Mapping[
    ...
    Callable[[Callable[..., Any], None, core_schema.CoreSchema],
             core_schema.CoreSchema],
] = {
...
_FIELD_VALIDATOR_F_MATCH: Mapping[
    tuple[FieldValidatorModes, Literal['no-info', 'general', 'field']],
    Callable[[Callable[..., Any], str, core_schema.CoreSchema],
             core_schema.CoreSchema],
] = {
